...
def SendRequest(url, method):
	...
	if method.lower() in ['put', 'post', 'get', 'delete']:
		...
		request = urllib2.Request('http://wiston999.dyndns.org:8765/%s'%(url))
...
if __name__ == '__main__':
	...
	while not end:
		...
		if command.lower().startswith('post'):
			response = SendRequest("%s/%s/%s"%(logingName, urllib.quote(command.split('##')[-2]), urllib.quote(command.split('##')[-1])), 'POST')
			ProcessResponse(response)
			
		elif command.lower().startswith('users'):
			response = SendRequest("%s/users"%(logingName), 'GET')
			...
			response = SendRequest("%s/search/%s"%(logingName, urllib.quote(command.split('##')[-1])), 'GET')
			if ProcessResponse(response):
			...
			response = SendRequest('%s/%s'%(logingName, command.split('##')[-1]), 'GET')
			if ProcessResponse(response):
				if len(response['comments']) > 0:
			...
			response = SendRequest(logingName.strip(), 'DELETE')
			ProcessResponse(response)
			end = True
	print "Bye."